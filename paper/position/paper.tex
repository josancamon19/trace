...
\paragraph{Cost comparison.} Table~\ref{tab:cost-comparison} presents a direct comparison of environment construction costs across methodologies. The break-even point for capture-replay tooling investment occurs at approximately 35 tasks; beyond this threshold, every additional task costs orders of magnitude less than replica construction.
...
\midrule
Speedup factor & --- & \textbf{120$\times$} \\
Break-even point\textsuperscript{*} & --- & $\sim$35 tasks \\
\bottomrule
\end{tabular}
\vskip 0.05in
...
\paragraph{Our response:} This is a data problem, not an environment-construction problem. \textbf{Multi-trajectory collection} captures multiple expert paths, providing recovery behaviors and alternatives. \textbf{Record-on-miss expansion} adds missing branches by capturing additional sessions when agents deviate. Exploration constraints are solvable with more capture, not handcrafted replicas.
...
\paragraph{Our response:} The six fully-replayable environments demonstrate \emph{replay quality}; our 71-task pilot study (Section~\ref{sec:pilot}) demonstrates \emph{collection scalability}. Together, they show that: (1) a single non-expert collector can gather 71 tasks in 10 hours; (2) the six refined environments replay deterministically on production sites; (3) challenging cases (authentication, payments, dynamic content) are included; (4) the 300-hour tooling investment built generalizable infrastructure, not task-specific code.

The pilot's captures require refactoring to match TRACE's current HAR-based format---but this illustrates rather than undermines our argument: when capture fails, recollection costs minutes. When replica construction fails, it costs weeks. Collection scalability and replay quality are now demonstrated; what remains is executing the collection at scale, which is an engineering effort, not a research question.
...
\paragraph{LM-based matching non-determinism.} TRACE uses LLM disambiguation when multiple responses match a request. This is a proof-of-concept convenience; production systems should default to deterministic matching via URL canonicalization and strict tie-breakers.
