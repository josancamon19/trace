...
\twocolumn[
\icmltitle{Position: Web Agent Research Should Prioritize Capture-Replay Infrastructure\\Over Handcrafted Replicas}
...
\paragraph{Single-trajectory coverage.} The current TRACE demonstration uses single-trajectory captures, validated via human traversal. We have not validated agent evaluation, where deviation from recorded paths causes missing responses. Supporting agent exploration requires multi-trajectory collection, which we propose but have not implemented—the most significant gap between our proof-of-concept and production-ready infrastructure.
